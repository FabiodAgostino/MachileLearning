...
//                    labelColumnName: nameof(ObjectImageData) + "." + coordinateType,
...
//            // Base pipeline per l'estrazione di feature
...
//            // Crea una lista per memorizzare le previsioni di ogni modello
//            var allPredictions = new List<List<float>>();
//            var groundTruth = new List<float>();

//            // Ottieni le previsioni da ogni modello
//            foreach (var model in models)
//            {
//                var predictions = model.Transform(testData);

//                var predictionValues = _mlContext.Data.CreateEnumerable<ObjectLocalizationEvalData>(
//                    predictions, reuseRowObject: false)
//                    .Where(x => x.Label == _objectType)
//                    .Select(x => coordinateType == "X" ? x.ScoreX : x.ScoreY)
//                    .ToList();

//                allPredictions.Add(predictionValues);

//                if (groundTruth.Count == 0)
//                {
//                    groundTruth = _mlContext.Data.CreateEnumerable<ObjectImageData>(
...
//            // Calcola le medie delle previsioni dell'ensemble
//            var ensemblePredictions = new List<float>();
//            for (int i = 0; i < allPredictions[0].Count; i++)
//            {
//                float sum = 0;
...
//            // Calcola manualmente le metriche
//            float mse = 0;
//            for (int i = 0; i < ensemblePredictions.Count; i++)
//            {
//                mse += (float)Math.Pow(ensemblePredictions[i] - groundTruth[i], 2);
//            }
//            mse /= ensemblePredictions.Count;
//            float rmse = (float)Math.Sqrt(mse);

//            // Calcola R²
//            float meanY = groundTruth.Average();
//            float ssTotal = groundTruth.Sum(y => (float)Math.Pow(y - meanY, 2));
//            float ssResidual = ensemblePredictions.Zip(groundTruth, (pred, truth) => (float)Math.Pow(pred - truth, 2)).Sum();
//            float rSquared = 1 - (ssResidual / ssTotal);

//            Console.WriteLine($"   {coordinateType} Ensemble - R² Score: {rSquared:F4}");
//            Console.WriteLine($"   {coordinateType} Ensemble - MSE: {mse:F4}");
//            Console.WriteLine($"   {coordinateType} Ensemble - RMSE: {rmse:F4}");

//            return new RegressionMetrics(l1: 0, l2: mse, rmse: rmse, lossFunction: 0, rSquared: rSquared);
//        }
...
//            // Estrai le coordinate in una colonna unica
//            var coordPipeline = featureExtractionPipeline
//                .Append(_mlContext.Transforms.Concatenate(
//                    outputColumnName: "Coordinates",
//                    nameof(ObjectImageData.X),
//                    nameof(ObjectImageData.Y)));

//            // Utilizza il modello di regressione LightGBM per la localizzazione unificata
//            var unifiedPipeline = coordPipeline
//                .Append(_mlContext.Regression.Trainers.LightGbm(
//                    labelColumnName: "Coordinates",
//                    featureColumnName: "NormalizedFeatures",
//                    numberOfIterations: 200,
//                    learningRate: 0.1,
//                    numberOfLeaves: 31))
//                .Append(_mlContext.Transforms.CopyColumns(
//                    outputColumnName: "UnifiedCoordinates",
//                    inputColumnName: "Score"));
